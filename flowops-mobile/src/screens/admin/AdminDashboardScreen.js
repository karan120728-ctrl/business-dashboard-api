...
export default function AdminDashboardScreen({ navigation }) {
  ...
  const loadStats = useCallback(async () => {
    try {
      ...
      // Stats endpoint might vary — graceful fallback
      setStats(null);
    } finally {
      setLoading(false);
      setRefreshing(false);
    ...
    >
      ...
      {loading ? (
        ...
        <View style={styles.statsGrid}>
          ...
          <StatCard icon="💰" label="Revenue" value={stats?.totalRevenue ? `$${Number(stats.totalRevenue).toFixed(0)}` : '—'} color="#f59e0b" />
...
const styles = StyleSheet.create({
  ...
  header: { flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center', marginBottom: 20, backgroundColor: '#4f46e5', borderRadius: 14, padding: 16 },
  ...
  statCard: { backgroundColor: '#fff', borderRadius: 12, padding: 14, flexDirection: 'row', alignItems: 'center', gap: 10, width: '47%', borderLeftWidth: 3, shadowColor: '#000', shadowOpacity: 0.04, shadowRadius: 8, elevation: 2 },
  ...
  navCard: { backgroundColor: '#fff', borderRadius: 12, padding: 16, flexDirection: 'row', alignItems: 'center', gap: 12, marginBottom: 10, shadowColor: '#000', shadowOpacity: 0.04, shadowRadius: 8, elevation: 2 },
